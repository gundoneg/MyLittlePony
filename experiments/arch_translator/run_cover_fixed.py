...
def run_alpha(a, args):
    ...
    print(f"\nalpha={a}  donor-acc s {sum(z['acc'] for z in s_raw)/n:.0%} d {sum(z['acc'] for z in d_raw)/n:.0%}"
          f"  deep-ablation {[f'{x:.0%}' for x in drops]}")

    # ---- OLD pipeline: per-depth align to own donor-0 via 'data', no scale_free ----
    ...
    # ---- FIXED pipeline: one common frame (shallow donor 0), scale_free ----
    ...
    # control: are features even read? (native C, mismatch interior features on target)
    fix_n_mm = zs(C_nat, cfg_d, feat_mismatch_zoo(d_fix[args.n_train:]))

    print(f"  coverage(worst-case)  OLD {cov_old[1]:.3f}  FIXED {cov_fix[1]:.3f}")
    print(f"  OLD    transfer L{args.shallow}->L{args.deep}  zs {old_t:.1%}")
