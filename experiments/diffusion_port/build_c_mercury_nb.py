...
Run-1 was negative: C's emitted weight delta U@Vt lived in the zoo's frame and landed in the
wrong basis on supra (no shared frame; the shuffled-signature control matched). Run-2 fix:
emit the per-block correction in each matrix's OWN SVD frame -- DW = U_r @ A(z) @ V_r^T, with
U_r,V_r the donor matrix's own singular vectors (intrinsic, cached) and A(z) a small r x r
correction C predicts from the gauge-invariant signature. Input gauge-INVARIANT, output
gauge-COVARIANT => the delta always lands in the model's current frame, by construction. No
model is rotated (RMSNorm-safe), so this is the correct "one frame" for zoo and supra.

Speedups (no quality loss): KV-cached self-generation; cached SVD; fewer donor steps.
...
cells.append(md(
"# supra50m → Mercury-2 diffusion LM **by the translator C alone** (run 2: SVD-frame deltas)",
"",
...
"",
"**Run-1 was negative** (B* ≈ floor; shuffled-signature control matched ⇒ C ignored",
"signatures): C's emitted weight delta lived in the zoo's basis and landed wrong on supra —",
"no shared frame. **Fix:** emit each block's correction in that matrix's **own SVD frame**,",
"`ΔW = Uᵣ · A(z) · Vᵣᵀ` (Uᵣ,Vᵣ = the donor matrix's intrinsic singular vectors, cached;",
"A(z) = a small r×r correction from the gauge-invariant signature). Input invariant, **output",
"covariant** ⇒ the delta always lands in the model's current frame *by construction*. No model",
"is rotated, so RMSNorm is preserved — this is the correct 'one frame' for zoo and supra.",
"",
...
"N_HELD    = 64",
"SEQ_LEN   = 256",
"ZOO_DEPTHS = [2, 3, 4, 2, 3, 4]",
"DONOR_STEPS = 800       # donors overfit the small corpus fast; 800 is plenty",
"DONOR_BS  = 16",
"C_STEPS   = 3000",
"C_BS      = 8",
"RANK      = 16          # SVD-frame correction rank (A is RANK x RANK)",
...
"    w = {'model.embed_tokens.weight': torch.randn(V, D, generator=g) * 0.02, 'model.norm.weight': torch.ones(D)}",
"    sh = {'self_attn.q_proj': (H*HD, D), 'self_attn.k_proj': (KV*HD, D), 'self_attn.v_proj': (KV*HD, D),",
"          'self_attn.o_proj': (D, H*HD), 'mlp.gate_proj': (FF, D), 'mlp.up_proj': (FF, D), 'mlp.down_proj': (D, FF)}",
"    for l in range(L):",
"        for n, s_ in sh.items(): w[f'model.layers.{l}.{n}.weight'] = torch.randn(*s_, generator=g) * 0.02",
...
"    w = make_llama(Lz, 100 + di); opt = torch.optim.AdamW(list(w.values()), lr=3e-4); t0 = time.time()",
"    for s_ in range(DONOR_STEPS):",
"        x = train_ids[torch.randint(0, train_ids.shape[0], (DONOR_BS,), device=DEV)]",
"        loss = F.cross_entropy(llama_forward(w, x[:, :-1], Lz).reshape(-1, V), x[:, 1:].reshape(-1))",
"        opt.zero_grad(); loss.backward(); torch.nn.utils.clip_grad_norm_(list(w.values()), 1.0); opt.step()",
"    zoo.append(({k: v.detach() for k, v in w.items()}, Lz))",
"    print(f'donor {di} (L={Lz}): AR CE {loss.item():.2f} ({time.time()-t0:.0f}s)')",
))
...
"",
"t0 = time.time()",
"zoo_cache = [svd_cache(w, Lz) for w, Lz in zoo]",
...
"    di = step % len(zoo); w, Lz = zoo[di]",
"    x0 = train_ids[torch.randint(0, train_ids.shape[0], (C_BS,), device=DEV)]",
"    t = sample_mask_rate(C_BS); x_t, m = forward_mask(x0, t)",
"    wB = C.emit(w, Lz, zoo_cache[di])",
"    loss = diffusion_loss(llama_forward(wB, x_t, Lz, causal=False, mask_row=C.mask_row(w)), x0, m, t)",
"    opt.zero_grad(); loss.backward(); torch.nn.utils.clip_grad_norm_(C.parameters(), 1.0); opt.step(); sched.step()",
"    ema = loss.item() if ema is None else 0.98 * ema + 0.02 * loss.item()",
"    if step % 500 == 0 or step == 1:",
"        with torch.no_grad():",
"            w0, L0 = zoo[0]",
"            fn = lambda i: llama_forward(C.emit(w0, L0, zoo_cache[0]), i, L0, causal=False, mask_row=C.mask_row(w0))",
"            hce = masked_ce_at(fn, held_ids, 0.5, n=16)",
"        print(f'step {step:>5}  train-ELBO(ema) {ema:6.3f}  zoo-donor0 masked-CE@0.5 {hce:5.2f}  ({time.time()-t0:.0f}s)')",
