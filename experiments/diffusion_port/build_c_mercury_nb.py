...
"               prior=None, beta=0.6, ban_ids=(0, 1, 2), temp_floor=0.3):",
"    \"\"\"run-4/5 sampler: cosine reveal (MaskGIT), Gumbel ranking (Dream), remasking, and",
"    run-5 fixes for the filler-confidence trap: PMI-debiased ranking",
"    conf = log p(tok) - beta*log prior(tok)  (reveal what is confident RELATIVE to base",
"    rate, not absolutely -- raw confidence reveals whitespace first and cascades),",
"    a temperature floor (no premature argmax collapse), and banned special ids.\"\"\"",
"    B, L = ids.shape",
...
"        if s < int(0.9 * steps): tau = max(tau, temp_floor)   # floor until the last 10%",
"        if tau > 0.05:",
...
"            conf = conf - beta * logp_prior[pred]              # PMI debias",
...
"print('\\n[B] v2 + PMI debias (128 steps):', repr(decode(g2[0])[:240]))",
"bos = torch.full((2, 1), 1, dtype=torch.long, device=DEV)",
"g3 = semi_ar_generate(fnB, bos, 129, block=32, steps_per_block=32, **kw)",
"print('\\n[C] semi-AR blocks + PMI (32x32 steps):', repr(decode(g3[0, 1:])[:240]))",
"# prompted continuation: pick the most COHERENT held prompts (lowest per-seq donor AR CE),",
"# not an arbitrary row (run-4 used a junk row by accident)",
"with torch.no_grad():",
"    lo = llama_forward(supra, held_ids[:32, :-1], L_SUPRA)",
"    ce_seq = F.cross_entropy(lo.reshape(-1, V), held_ids[:32, 1:].reshape(-1),",
"                             reduction='none').view(32, -1).mean(1)",
"best = ce_seq.argsort()[:2]",
"prompt = held_ids[best, :32]",
"g4 = semi_ar_generate(fnB, prompt, 32 + 96, block=32, steps_per_block=32, **kw)",
"for b in range(2):",
