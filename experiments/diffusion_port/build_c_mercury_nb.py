"""Builds c_mercury_port.ipynb — THE PROJECT-METHOD port: supra50m (AR Llama) -> Mercury-2-
style diffusion LM where model B is produced EXCLUSIVELY by the weight-translator C.

Method contract (per the project's thesis):
  * B receives ZERO training. B* = C(supra50m): the donor's own weights + per-block low-rank
    deltas EMITTED by C from gauge-invariant block signatures + an emitted [MASK] embedding.
  * The only data anywhere is sampled from the donor itself (the allowed 'Q-A captured from
    the model' budget); it trains the small zoo and C — never B*.
  * C is per-block and weight-tied (depth-agnostic), trained through the diffusion loss on a
    zoo of small AR Llamas, then applied zero-shot to the 12-layer donor (phase-10: per-slot
    corrections are small/low-rank [E1], the tied per-block translator extrapolates over
    depth where computation is non-recursive [E0/E3]).
...
# ---------------- cell 3: tokenizer + functional Llama ----------------
cells.append(code(
...
# ---------------- cell 4: self-generated corpus (the entire data budget) ----------------
cells.append(code(
"@torch.no_grad()",
...
# ---------------- cell 9: ZERO-SHOT application to supra50m + evals ----------------
cells.append(code(
"with torch.no_grad():",
