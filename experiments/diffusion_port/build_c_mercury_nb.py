...
"               temp_floor=0.3):",
"    \"\"\"run-6 sampler. Against the filler trap, debias WHAT is predicted, not only the",
"    reveal order: subtract samp_beta*log(prior) from the logits BEFORE softmax (frequency-",
"    penalized sampling) and apply a repetition penalty rep_gamma*log1p(count in current",
"    text). Reveal ranking additionally PMI-debiased by beta; temperature floor; banned",
"    special ids; cosine schedule; Gumbel ranking noise; remasking.\"\"\"",
"    B, L = ids.shape",
...
"            logits = logits - samp_beta * logp_prior      # frequency-penalized prediction",
"        if rep_gamma > 0:",
"            for b in range(B):                             # repetition penalty per row",
...
"kw = dict(temp0=0.9, alg_temp=0.3, remask_frac=0.15, prior=UNI, beta=0.6,",
"          samp_beta=0.4, rep_gamma=0.8)",
...
"- **Sampler comparison [A]/[B]/[C]**: run-4 exposed the *filler-confidence trap* — in a sea",
"  of masks the most-confident tokens are whitespace/specials, raw confidence reveals them",
"  first and they cascade ([B] produced all-newlines, [C] all-spaces). Run-5 fixes: **PMI-",
"  debiased ranking** (confidence relative to the corpus unigram prior), a **temperature",
"  floor** (no premature argmax collapse), **banned special ids**, and prompts selected by",
"  coherence (lowest donor AR CE) instead of an arbitrary held row.",
