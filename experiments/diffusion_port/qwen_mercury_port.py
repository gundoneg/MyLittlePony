"""Qwen3.5-9B -> Mercury-2 diffusion by translator C (v6: bf16 + NaN-guards + finite dry-run).
...
BUILD = 'v6-bf16-nanguard'   # bump on every change; printed below so the running version is unmistakable
print('=' * 60)
...
model = AutoModelForCausalLM.from_pretrained(
    MODEL_ID, torch_dtype=torch.bfloat16, trust_remote_code=True,   # bf16: fp32-range, no overflow->inf
...
def _conv1d_fp32(inp, weight, bias=None, *a, **k):
    if inp.dtype in (torch.float16, torch.bfloat16):    # T4 has no half/bf16 conv engine
...
def denoise_v2(fn, ids, frozen, steps=64, temp0=1.0, alg_temp=0.3, remask_frac=0.15, prior=None,
    ...
    for s in range(steps):
        ...
        if n_left == 0: break
        logits = torch.nan_to_num(fn(ids).float(), nan=-1e9, posinf=1e4, neginf=-1e9)  # never feed NaN to multinomial
...
class TranslatorC(nn.Module):
    ...
    def mask_row(self):
        # fp32, chunked: softmax weight 1/V=4e-6 is below fp16 min-normal; chunking avoids a
        # 4GB E_W.float() materialization. Result cast back to the model dtype.
        p = self.mask_logits.float().softmax(0).to(E_W.device)
        out = torch.zeros(E_W.shape[1], device=E_W.device, dtype=torch.float32)
        for i in range(0, E_W.shape[0], 16384):
            out = out + p[i:i + 16384] @ E_W[i:i + 16384].float()
        return out.to(E_W.dtype)
    def _z(self, mm):
...
C.uninstall()
# FINITENESS asserts: the real value of the dry-run -- catch NaN/Inf in MINUTES, not after
# the corpus + training. Check the loss, the mask embedding, and a full-depth forward's logits.
assert torch.isfinite(ld), f'dry-run loss not finite ({ld.item()}) -- numeric instability (dtype?)'
assert torch.isfinite(C.mask_row()).all(), 'mask_row has NaN/Inf'
C.install(L_FULL)
lg = run_stack(x0d, L_FULL, causal=False, mask_row=C.mask_row())
assert torch.isfinite(lg).all(), 'full-depth B* logits have NaN/Inf (fp16 overflow?)'
C.uninstall(); del lg
del x0d, x_td, md_, kdp, kdi, ld, ids_d, fr_d
gc.collect(); torch.cuda.empty_cache()
_DRYRUN_OK = True
print('PIPELINE DRY-RUN OK: train step + eval + sampler finite, exercised end-to-end on both GPUs')
...
ema, t0, nskip, run = None, time.time(), 0, 0
for step in range(1, C_STEPS + 1):
    ...
    if not torch.isfinite(loss):                      # NaN-guard: skip, never train on NaN
        opt.zero_grad(set_to_none=True); C.uninstall(); nskip += 1; run += 1
        if run > 25: raise RuntimeError(f'training diverged to NaN ({nskip} skipped) — lower LR or check dtype')
        continue
    run = 0
    ...
    if step % EVAL_EVERY == 0 or step == 1:
        with torch.no_grad():
            ...
            C.uninstall()
        em = ema if ema is not None else float('nan')
        print(f'step {step:>5}  train(ema) {em:6.3f}  sub-stack-L{Lp} masked-CE@0.5 {hce:5.2f}  skipped {nskip}  ({time.time()-t0:.0f}s)')
print(f'C trained — the donor was never trained, only C was (NaN steps skipped: {nskip})')
