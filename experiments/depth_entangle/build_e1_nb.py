...
"",
"**Setup.** Add the `supra50m` checkpoint folder (config.json + model.safetensors +",
"tokenizer.json) as a Kaggle dataset, enable a GPU accelerator, and Run All. No `transformers`",
"needed — a minimal CUDA Llama forward is inlined.",
))
...
"torch.manual_seed(0)",
"",
"# locate the supra50m checkpoint (uploaded as a Kaggle dataset)",
"CANDS = glob.glob('/kaggle/input/**/config.json', recursive=True)",
"MODEL_DIR = os.path.dirname(CANDS[0]) if CANDS else 'supra50m'",
"",
"EXEMPLAR = 6                 # shared exemplar layer grafted into the interior",
...
"        self.hd, self.d = cfg['head_dim'], cfg['hidden_size']",
"        self.L, self.eps = cfg['num_hidden_layers'], cfg['rms_norm_eps']",
"        self.theta, self.vocab = cfg['rope_parameters']['rope_theta'], cfg['vocab_size']",
...
"            hs.append(x)",
"        logits = self.rms(x, self.w['model.norm.weight']) @ self.embed.T",
...
"print(f'loaded: L={m.L} d={m.d} H={m.H}/{m.KV} vocab={m.vocab}')",
))
